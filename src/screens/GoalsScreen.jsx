...
import { tk, CAT, CATL } from "../constants";
...
export default function GoalsScreen({ s, dp }) {
  ...
  function add() {
    ...
    setF({ title: "", cat: "Work", deadline: "", status: "Not Started", ms: "" });
    setOpen(false);
  }
        ...
        {cats.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => setFilter(cat)} style={{
              ...
              {cat !== "All" && <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: CAT[cat], marginRight: 7, verticalAlign: "middle" }} />}
              {cat}
            </button>
          );
        })}
        ...
        return (
          <div key={cat}>
            <div onClick={() => setFilter(filter === cat ? "All" : cat)} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, cursor: "pointer", transition: "opacity 0.15s" }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", background: CAT[cat], boxShadow: filter === cat ? `0 0 8px ${CAT[cat]}60` : "none", transition: "box-shadow 0.2s" }} />
              ...
              {gs.map(g => (
                <Card key={g.id}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                    <div style={{ flex: 1 }}><PBar v={g.prog} color={CAT[cat]} /></div>
                    <Mono size={11} color={CAT[cat]} style={{ width: 30, textAlign: "right" }}>{g.prog}%</Mono>
                  </div>
                  <input type="range" min={0} max={100} value={g.prog} onChange={e => dp({ type: "GOAL_P", id: g.id, v: +e.target.value })}
                    style={{ "--thumb": CAT[cat], "--track": d ? tk.d3 : tk.cream3, accentColor: CAT[cat] }} />
                  {g.ms?.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
                      {g.ms.map((m, i) => <span key={i} style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, padding: "4px 10px", borderRadius: 20, background: d ? tk.d3 : CATL[cat], color: CAT[cat] }}>{m}</span>)}
                    </div>
                  )}
                </Card>
              ))}
